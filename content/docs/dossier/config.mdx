...
配置后，WebUI 左侧 Dossier 入口会打开该目录下的 `report.md` 作为默认报告，静态文件通过 `/dossier/` 路径提供。
...
- 目录下存在 `index.html`
- 目录下存在 `report.md`（左侧 Dossier 默认入口所需；Artifacts 打开的其他 `.md` 报告只需要 Dossier 运行时可用）
...
<Callout title="默认报告边界" type="info">
  默认 Dossier 报告可以位于服务端工作目录外，但必须在 `webui.dossierDir` 目录内。即使当前处于 WebUI Project 视图，左侧 Dossier 入口也不会自动改成 Project 目录下的 `report.md`。SIREN 会解析符号链接，阻止默认报告逃逸到 Dossier 目录外。
</Callout>
