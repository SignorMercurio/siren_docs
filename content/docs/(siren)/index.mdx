...
<Accordions>
  <Accordion title="服务端部署">
    ...
    SIREN Server 默认通过 `443` 接收客户端连接，并在 `8080` 提供 [Web 控制台](/webui) 和 [MCP 服务](/mcp)。如需修改端口，请参考 [配置文件](/config)。
...
if ($PSVersionTable.PSVersion -lt [version]'5.1') { throw 'PowerShell 5.1 or later is required' }; [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; iwr -Uri 'https://oss-siren.oss-cn-hongkong.aliyuncs.com/get_siren.ps1' -UseBasicParsing | iex
```
