...
Clean 不自动修改云端安全组。若曾通过新版 Configure egress 配置出站访问，清理回报成功或已安排删除后，Clients 页面会提供 Restore egress 快捷入口。请手动恢复；客户端离线不影响服务端执行恢复。共享规则、人工修改冲突及历史记录限制见 [Web 控制台](/webui#clients)。
