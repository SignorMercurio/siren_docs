...
<Callout title="安全建议" type="warn">
  OSS 凭证只用于本地模式的 `siren recon` 与 `siren upload`。远程 Recon、文件拉取和 Artifacts 浏览不依赖 OSS 凭证。SOAR 凭证只在 SIREN Server 调用 SOAR 能力时需要。不要把 Access Key 写入 Git 仓库、日志或共享配置文件。
</Callout>
