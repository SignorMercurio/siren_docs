...
SIREN 支持本地模式和远程模式。本地模式直接在目标主机上运行，适合离线单机排查；远程模式由 SIREN Server 管理在线客户端，提供服务端 REPL、Web 控制台、Artifacts、Projects、Agentic IR 和 MCP 服务。需要内部 SOAR 能力时，还可以放行、部署目标主机，或调查 AccessKey、获取告警详情。
...
<Cards>
  <Card title="信息收集" icon={<Search />} href="/recon">
    采集系统、用户、进程、网络、计划任务、服务、文件、Rootkit 和应用配置等信息，生成 Markdown 报告
  </Card>
