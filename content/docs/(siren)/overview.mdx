...
<Cards>
  ...
  <Card title="Agentic IR 与 MCP" icon={<Brain />} href="/air">
    在 Web 控制台中启动 Claude / Codex / Qoder / Pi 会话，或在服务端 REPL 中启动 Claude 会话，并通过 MCP 使用 SIREN 工具
  </Card>
...
在控制主机运行 `siren_server`，目标主机运行 `siren client` 并通过 TLS 连接服务端。服务端负责客户端管理、命令下发、Artifacts 保存、Project 目录隔离、Web 控制台和 MCP 服务；Dossier、SOAR、OSS、Codex / Claude / Qoder / Pi CLI 等能力按需接入。
