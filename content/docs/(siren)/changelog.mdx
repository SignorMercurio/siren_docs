...
- **WebUI Agentic IR 改为浏览器内嵌终端**：Agentic IR 页面直接在浏览器中运行真实的 Claude Code / Codex 终端（TUI），替代原先的结构化对话视图；可以像在本地终端一样与 Agent 交互、回答其提问并查看完整输出
- **AIR 会话由服务端托管并持续运行**：刷新浏览器、切换页面或短暂断连后可重新接入并回放此前的终端输出，Agent 进程不中断；仅在主动停止或进程自身结束后才回收
- **Agentic IR 入口整合为 Manual / Custom / Event / Resume 四种模式**：Manual 启动不带 SIREN 上下文的裸 Claude / Codex 终端用于临时 CLI 操作，Custom 传入自定义 prompt，Event 拉取告警事件，Resume 恢复历史会话；四种模式均可选择 Claude 或 Codex Provider
- **WebUI 浏览器终端零额外配置**：与浏览器 Shell 一致，只受 WebUI 全局鉴权保护，复用 `air.providers.<name>` 的 CLI 配置(Manual 模式取 `cliPath` + `args`)，没有单独的 enable / 鉴权 / Provider 开关

### Improvement

- **AIR 不再替操作者决定大模型 CLI 的行为**：SIREN 只注入项目作用域的 SIREN MCP 服务、种子 prompt 与恢复会话 ID；模型、推理强度、权限、沙箱、审批（含 MCP 工具审批）一律沿用操作者自己的 Claude Code / Codex 配置
- **精简 AIR 配置**：移除 `air.providers.claude` 的 `model` / `effort` / `allowedTools` 与 `air.providers.codex` 的 `model` / `effort` / `sandbox` / `approval` / `mcpApproval` / `enabledTools`；Claude 仅保留 `cliPath` / `strictMCPConfig` / `args`，Codex 仅保留 `cliPath` / `args`。`strictMCPConfig` 现在只作用于 WebUI 终端注入的 `--mcp-config`
- 移除旧的结构化 AIR 会话接口 `/api/air/stream` 与浏览器端对话组件，改由内嵌终端承载
- Running / History 列表以最初的 prompt 作为主标题（时间随后展示）；恢复某条历史会话后，它会从 History 去重并出现在 Running，标题沿用原 prompt
- 服务端 REPL 的 `air` 命令继续保持 tmux 交互式体验，并同样改为继承环境的 MCP 配置（不再单独传 `--strict-mcp-config`）
- Agentic IR 表单控件（含 Client 选择器）统一为深色设计系统样式

### Bugfix

- 修复切换到尚未在服务端创建目录的 Project 时，Agentic IR 历史列表报 `resolve project directory ... no such file or directory` 的问题；此类 Project 现在展示为空历史
- 修复停止已自行结束的 AIR 会话时可能返回错误的问题（终端关闭改为幂等）

## v2.11.0 <DateTag>2026-06-29</DateTag>

### Feature

- **WebUI 新增 Projects 工作区**：可创建、重命名、删除 Project，并为每个 Project 固定一个服务端目录；Global 视图保留全局资源视角，Project 视图只显示和操作该 Project 的客户端与取证目录
- **客户端 Project 归属管理**：Clients 列表可手动把客户端归属到某个 Project 或移回未归属；归属绑定客户端稳定 AgentID，客户端重连后继续生效
- **Project 视图下部署自动归属**：在某个 Project 视图中执行 WebUI 单机或批量部署时，部署目标下一次连接会自动归属到该 Project；服务端 REPL 部署保持原有全局行为
...
- **Artifacts 支持新建目录**：Artifacts 页面新增 New Folder 操作，可在当前目标目录下创建服务端目录，并复用同一套路径安全校验
- **批量部署支持目标备注**：服务端 REPL 与 WebUI deploy-batch 目标格式扩展为 `instanceId[,publicIp[,note]]`，可在部署时为目标预置备注；只填写备注时可写成 `instanceId,,note`

### Improvement

- Artifacts 支持 Project 作用域：Global 视图继续浏览 `siren_server` 工作目录；Project 视图只浏览该 Project 的固定目录，并沿用绝对路径、穿越、点文件、symlink 和特殊文件防护
- WebUI AIR 在 Project 视图下使用 Project 目录作为 Provider working directory，并按该目录过滤历史会话；注入给 AIR 的 SIREN MCP 会限制 `ls` / `run` 只能访问当前 Project 客户端，Global MCP 行为不变
- Recon 客户端回传报告时会按客户端 Project 归属保存：已归属客户端写入 Project 目录并保留原始 `SIREN_*.md` 文件名，未归属客户端继续写入全局工作目录
- Dossier 左侧默认入口继续打开 `webui.dossierDir/report.md`；从 Project Artifacts 打开的 Markdown 报告仍可进入 Dossier 编辑，并保持文件和粘贴资源在 Project 目录内
- 新增 `webui.projectsBaseDir` 配置和 `--webui-projects-base-dir` 参数，用于限制 Project 目录创建位置；Project 重命名只改显示名，不移动已有目录
- AIR 配置改为 `air.providers.<name>` 结构，Claude 与 Codex 可分别配置 CLI 路径、模型、推理强度和额外参数；示例配置默认 Claude `claude-opus-4-8`，Codex `gpt-5.5` + `xhigh`，Codex 使用 `workspace-write` sandbox 与 `mcpApproval: approve`
- WebUI AIR 继续使用结构化 `/api/air/stream` 会话；旧的浏览器 PTY AIR 路由已移除，服务端 REPL 的 `air` 命令保持原有交互式体验
- AIR 状态展示补齐工具调用状态、Claude rate-limit 百分比和更清晰的流式错误提示，便于判断会话是否仍在运行
- WebUI 进程分析详情面板展示更清晰，并限制为 Linux 客户端入口，避免 Windows 客户端触发不可用查询
- 服务端 REPL 的 `note <Client ID>` 现在可省略备注内容来清空已有备注，同时保留多词备注

### Bugfix

- 修复 MCP `run` 工具在慢命令场景下提前判定超时的问题；等待时间现在跟随 `recon.commandTimeout` 并保留额外缓冲
- 加强 MCP `run` 默认命令黑名单：阻断 `find -delete`、`truncate`、`shred`、`wipefs`、块设备写入、服务重启/重载、防火墙清空、账号/组变更等破坏性操作，并检查 `sh` / `bash` / `dash` / `zsh` / `ksh -c '...'` 包装后的 payload
- 修复 MCP 黑名单对普通引号说明文本的误报，例如 `echo "rm -rf /tmp/x"` 不再因为字符串内容被拒绝
- 修复客户端 Files 中指向目录的 symlink 被标记为普通文件的问题，浏览器文件树可继续按目录展开和下载
- 修复 WebUI 进程分析错误状态不准确的问题：非 Linux 客户端返回 422，超时返回 504，其他通信失败返回 502
- 修复 Recon 报告文件名校验，服务端只接受 `.md` 报告文件名，避免回传报告覆盖服务端工作目录中的非报告文件
- 修复 Project 视图下 Recon 报告仍落到全局工作目录的问题
- 修复移动端底部导航中 Files 与 Artifacts 标签重复的问题
- 修复 Recon 文件完整性检查中的包管理器检测偏差，并移除 SSH 暴力破解排查命令中的异常字面量
- 修复生成的 artifact 文件名可能包含 `:` 导致跨平台路径不兼容的问题
- 修复部分协议输出和文件预览中的二进制内容识别边界，减少错误预览和挂起风险
...
- SOAR 平台凭证优先支持独立的 `SOAR_ACCESS_KEY_ID` / `SOAR_ACCESS_KEY_SECRET`，同时保留 Alibaba Cloud SDK 默认凭证链兼容；在服务端 REPL 中更新任一 SOAR 凭证变量都会自动重新初始化客户端
