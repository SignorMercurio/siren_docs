...
## v2.33.0 <DateTag>2026-09-23</DateTag>

### Features

- 新增 **Reports** 页面替代左侧的 Dossier 外链：列出当前范围内符合 Dossier 规范的 Markdown 应急报告，Global 视图汇总工作目录和所有 Project 并标注归属；行尾 **Open in Dossier** 打开编辑，**Blank template** 打开只读默认模板，**Rescan** 手动重新扫描

### Improvements

- Artifacts 预览中的 Rename 按钮移到 Download 右侧，与右键菜单顺序一致

## v2.32.10 <DateTag>2026-09-22</DateTag>

### Features

- AIR GUI 支持编号证据脚注：点击编号查看来源说明，并从卡片打开已保存的原文；每条回复下方保留可展开的来源列表
- Claude Code、Codex、Qoder 与 Pi 的新建和恢复 GUI 会话采用统一证据约定，说明数据来源、采集时间和查看位置，并在权限允许时保存重要的临时命令或查询结果
- Markdown 报告预览支持脚注和同一工作区内的相对文件链接；导出会话时隔离各条消息的脚注编号，避免来源串位

### Improvements

- 桌面侧栏默认折叠为图标，可展开并记住当前浏览器的选择；悬停或键盘聚焦图标时显示名称，移动端保留完整抽屉导航
- 调整 AIR 会话列表搜索框的间距、文字和聚焦样式

## v2.32.9 <DateTag>2026-09-22</DateTag>

### Improvements

- AIR 与设置页的 Provider 选项统一按 Codex、Claude、Pi、Qoder 排列，将 Qoder 移到最后；保留已保存的 Provider 偏好

## v2.32.8 <DateTag>2026-09-21</DateTag>

### Bug Fixes

- 修复 SOAR 明确限流拒绝被误判为提交结果未知、阻塞同账号出站规则恢复的问题；保留网络超时等不确定结果的重复提交保护
- 恢复失败时指出实际阻塞的实例、操作和步骤，避免将其他实例的异常误认为当前实例的问题

### Improvements

- Restore egress 移除常驻 Verify 按钮，仅对有执行 ID 的未决步骤提供 **Recheck execution**；缺少执行 ID 时提示人工核对，正常记录保留恢复操作
- 重新查询只核对当前记录，恢复前仍检查同账号未完成的云端操作
- 调整恢复列表的窄屏布局，避免实例 ID 被挤成竖排

## v2.32.4 <DateTag>2026-09-17</DateTag>
...
## v2.32.3 <DateTag>2026-09-16</DateTag>

### Improvements

- 客户端启动后前 60 秒内，首次连接失败后以 2 秒间隔重试，减少自动部署放行后的上线等待；首次连接成功或窗口结束后恢复原有退避策略

## v2.32.2 <DateTag>2026-09-16</DateTag>
...
## v2.32.1 <DateTag>2026-09-16</DateTag>
...
## v2.32.0 <DateTag>2026-09-16</DateTag>
...
## v2.31.0 <DateTag>2026-09-15</DateTag>
...
## v2.30.5 <DateTag>2026-09-15</DateTag>
...
## v2.30.2 <DateTag>2026-09-14</DateTag>
...
## v2.30.1 <DateTag>2026-09-11</DateTag>
...
## v2.30.0 <DateTag>2026-09-10</DateTag>
...
- MCP 的 `run`、`recon` 等按客户端操作的工具可直接使用 Instance ID、内网 IP 或主机名指定目标；匹配到多台主机时返回候选 ID
- 新增 MCP 工具 `run_batch`，在当前 Project 的多台客户端上并发执行同一命令并按主机汇总，每台主机都有独立的 `audit_id` 供 `read_run_result` 和 `search_run_result` 回读
...
## v2.29.0 <DateTag>2026-09-09</DateTag>
...
## v2.28.0 <DateTag>2026-09-08</DateTag>
...
## v2.27.7 <DateTag>2026-09-08</DateTag>
...
## v2.27.6 <DateTag>2026-09-07</DateTag>
