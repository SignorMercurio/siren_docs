...
## v2.18.1 <DateTag>2026-08-07</DateTag>

### Improvement

- Logs 页面每条日志固定占一行：结构化字段紧随正文，过长的值就地省略，点击整行展开完整内容；级别符号按类型着色，成功与普通信息不再同色，警告和错误整行带底色便于定位
- AIR GUI 正文行宽放宽到与表格、代码块同宽，提问气泡的右边界与输入框对齐
- 左侧导航中 Logs 移到 Projects 之后

## v2.18.0 <DateTag>2026-08-07</DateTag>

### Feature

- Web 控制台新增 Logs 页面：直接在浏览器查看服务端运行日志，支持按级别和组件过滤、按关键词搜索日志正文与字段值，可实时跟随或暂停，并复制、下载当前内容。日志保留在内存中，条数由 `logging.bufferSize` 控制（默认 5000，设为 `0` 关闭该页面），重启后清空
- AIR GUI 会话在等待操作员决策时给出明确提示：待处理的审批或提问会标记在标签页、时间线导航和输入框上方，可一键跳转到对应请求；连接重试耗尽后显示断线提示并支持手动重连

## v2.17.6 <DateTag>2026-08-06</DateTag>

### Bugfix

- 服务端发行包不再携带发布机的 TLS 证书私钥；首次安装改为在控制主机生成独立自签证书，后续升级会保留现有配置与证书
- 修复客户端重连时旧端口转发仍可能向新控制连接发送数据的问题；转发连接现在严格随创建它的控制会话结束
- 修复 WebUI 切换项目后 Investigations 旧请求结果回填到新项目的问题；AccessKey、Recon 和进程分析结果现在只更新发起时的项目视图
- 统一 AIR GUI 与 Terminal 的断线重试判定：连接稳定后才恢复重试额度；服务端退出时所有 AIR 会话共用一次关闭期限并行收敛

## v2.17.5 <DateTag>2026-08-05</DateTag>

### Improvement

- MCP 服务兼容最新 `2026-07-28` 协议，当前客户端使用无会话的 Streamable HTTP 交互；仍保留旧版初始化协商，并拒绝不安全的跨域浏览器请求

## v2.17.4 <DateTag>2026-08-05</DateTag>

### Feature

- 重构 GUI 提问卡片：问题标题与正文分层显示并保留换行，选项改为带序号和说明的纵向列表；即使 Agent 提供了选项，也可以直接输入自定义答案

### Bugfix

- 修复从只读历史记录继续 GUI 会话时原时间线消失的问题；历史内容会保留在当前标签页中，新消息接续显示
- 改善宽屏下 GUI 时间线的内容宽度和 Markdown 表格可读性；正文保持舒适行长，表格、代码和操作记录可利用更宽空间，过宽表格改为横向滚动而不是挤压列宽
