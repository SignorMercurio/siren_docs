...
```plaintext
时间轴: 2025-11-05 10:10:40 → 11:19:43 (持续70分钟)
...
**受攻击接口**: `/esign/rest/cfg/timestamp`

**注入位置**: HTTP请求头 `Content-Type` 字段

**触发机制**: LogFilter 拦截器记录请求日志时触发Log4j2解析
...
```bash
# sign-web-api 主进程
vale  16912  5.8% 11.8%  运行时长: 约15000小时 (自2024年启动)
父进程: systemd (PID 1)
子进程: 无异常
