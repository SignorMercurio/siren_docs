---
title: Agentic 应急响应
description: 在 WebUI 中启动 Claude / Codex / Qoder / Pi，或在服务端 REPL 中启动 Claude 辅助排查会话
icon: Brain
---
...
<Callout title="前置条件" type="info">
  需要在服务端配置可用的 Claude、Codex、Qoder 或 Pi CLI 路径（`air.providers.<name>.cliPath`），并启用 [MCP 服务](/mcp#启用与访问)。Event 模式需要[设置 SOAR 平台访问凭证](/credential#设置-soar-平台访问凭证)。服务端 REPL 的 `air` 命令固定使用 Claude，并需要 tmux。
</Callout>
...
<Callout title="配置边界" type="info">
  - **SIREN 只做**：使用 `air.providers.<name>.cliPath` 和 `args` 启动对应 CLI，为所有入口注入当前范围的 SIREN MCP，并按需加入种子 prompt 或恢复会话 ID；GUI 只使用 Provider 公开的机器协议。
  - **Claude、Codex、Qoder**：模型、权限、沙箱、普通 Shell 审批和操作者已有 MCP 的工具确认规则由 CLI 自身决定。
