...
<Callout title="前置条件" type="info">
  需要在服务端配置可用的 Claude、Codex、Qoder 或 Pi CLI 路径（`air.providers.<name>.cliPath`），并启用、配置独立凭证的 [MCP 服务](/mcp#启用与访问)。Event 模式需要[设置 SOAR 平台访问凭证](/credential#设置-soar-平台访问凭证)。服务端 REPL 的 `air` 命令固定使用 Claude，并需要 tmux。
</Callout>
