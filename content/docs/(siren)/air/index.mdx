...
Agent 通过 [MCP 集成](/mcp) 使用 `ls`、`run`、`recon` 获取客户端线索。用户明确要求向某台 ECS 部署 SIREN 时，Agent 也可以调用 `deploy` 并等待客户端实际连接；Codex AIR 会自动批准这次工具调用，调查或诊断请求本身不构成部署授权。操作员可以在终端中继续追问或调整方向。
