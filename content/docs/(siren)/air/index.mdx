...
<Callout title="配置边界" type="info">
  WebUI AIR 只使用 `air.providers.<name>.cliPath` 和 `args` 启动对应 CLI，并在 Custom、Event、Resume 模式注入 SIREN MCP、种子 prompt 或恢复会话 ID。模型、权限、沙箱、普通 Shell 审批和操作者已有 MCP 的工具确认规则由 Claude Code / Codex 自身配置决定。Codex Provider 会把 SIREN 注入的 MCP 工具设置为默认允许，避免每次 SIREN 工具调用都弹出确认；如需限制其他可用工具，请在对应 CLI 的用户配置中处理。
</Callout>
