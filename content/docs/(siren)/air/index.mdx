...
<Callout title="前置条件" type="warn">
  ...
  - WebUI AIR 会使用 SIREN MCP 工具，请确认 [MCP 服务](/mcp#配置)已启用
...
通过服务端 REPL 使用 `air` 命令时，SIREN 会通过 tmux 启动一个新面板并自动运行配置的大模型 CLI（e.g. Claude Code）并提供预定义的 prompt 作为输入：
...
在 [Web 控制台](/webui#agentic-ir) 的 Agentic IR 页面中选择客户端后，也可以使用告警事件模式或自定义 Prompt 启动 AIR。WebUI 会按顺序展示 AI 回复、工具调用和结果，并支持在同一会话中继续追问。
...
将告警信息和预定义 prompt 一起传递给配置的大模型 CLI（如 Claude Code），在服务端启动 Agent。服务端 REPL 模式使用 tmux 新面板；WebUI 模式会把运行过程整理成浏览器中的会话视图。
