...
也可以切换到 Resume 模式，恢复当前 WebUI 视图工作目录对应的历史会话。Global 视图使用 `siren_server` 工作目录；Project 视图使用该 Project 的固定目录。Claude 会读取 Claude Code history；Codex 会读取对应目录下的 Codex session history。WebUI 只展示解析后的历史消息，不会向浏览器暴露本机 JSONL 文件路径。

<Callout title="Project 作用域" type="info">
  在 Project 视图下启动 WebUI AIR 时，Provider working directory 会切到当前 Project 目录，历史会话也只按该目录过滤。WebUI 注入的 SIREN MCP 工具只会列出当前 Project 客户端，并拒绝对其他 Project 客户端执行命令。Global 视图保持全局行为。
</Callout>
