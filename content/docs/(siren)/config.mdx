...
recon:
  ...
  profiles:
    ...
    hunt: [user, ssh, process, network, cron, service, env, rootkit, application, topology, config, activity, credential]
...
没有 `recon.profiles` 的旧配置会把已启用的传统 `recon.<module>` 与旧式 recon 插件合成为唯一的 `default`，并记录迁移提示。`topology`、`config`、`activity`、`credential` 以及 Linux 的 `supplychain` 属于 opt-in 采集，不会被旧配置或无配置 fallback 自动启用；内置 `hunt` 包含前四项，但不会自动启用 `supplychain`。新旧结构同时存在时，以 Profile 为准，旧键被忽略。旧配置中残留的 `rules` 不影响启动，但会记录一次已忽略提示；这些规则不再参与 Recon。无效的远程 `reload` 会保留上一个有效 Profile 快照。
