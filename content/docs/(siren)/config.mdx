...
| `webui.dossierDir` | string | `/opt/siren/dossier` | Dossier 编辑器文件目录，可位于服务端工作目录外；左侧 Dossier 入口默认打开该目录下的 `report.md`。留空、目录不可用或默认报告不可访问时，WebUI 会提示错误 |
...
| `air.prompt` | string | - | 告警事件模式下的 prompt 模板（支持 `%s` UID、`%s` 云安全中心告警 ID、`%d` Client ID） |
