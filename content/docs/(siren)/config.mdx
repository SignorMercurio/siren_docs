...
| `webui.dossierDir` | string | -（示例配置为 `/opt/siren/dossier`） | Dossier 编辑器文件目录，可位于服务端工作目录外；Reports 页面的 **Blank template** 以只读模板方式打开该目录下的 `report.md`，实际报告需从 Artifacts 或 Reports 进入后保存。目录留空、不可用或默认报告不可访问时，WebUI 会提示错误。详见 [Dossier 配置](/dossier/config) |
...
启用 `mcp.enabled` 前，必须通过受保护的服务环境配置 `SIREN_MCP_ADMIN_TOKEN` 和 `SIREN_MCP_RAVEN_TOKEN`，两者须非空且不同，不能写入 YAML 或项目文件。前者供运维客户端使用，后者仅用于 Raven 申请群项目凭证。未配置时服务端拒绝启动，不降级为匿名访问。AIR 自动注入当前会话凭证；外部客户端的 Header 和迁移步骤见 [MCP 集成](/mcp#启用与访问)。
