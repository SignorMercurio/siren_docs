...
| `air.provider` | string | `claude` | WebUI [Agentic 应急响应](/air)默认使用的 Provider，可选 `claude` 或 `codex`；页面中也可在每次启动会话时选择 |
...
| `air.providers.claude.model` | string | `claude-opus-4-8` | Claude Provider 使用的模型 |
...
| `air.providers.codex.sandbox` | string | `workspace-write` | Codex 沙箱模式 |
| `air.providers.codex.approval` | string | `never` | Codex approval 模式 |
| `air.providers.codex.mcpApproval` | string | `approve` | Codex 访问 SIREN MCP 工具时使用的默认审批模式 |
...
<Callout title="v2.10.2 AIR 配置更新" type="info">
