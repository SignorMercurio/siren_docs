...
`logging.bufferSize` 额外在内存中保留最近若干条日志，供 [Web 控制台 Logs 页面](/webui#logs)查看。保留的内容同样受 `logging.level` 约束，`logging.output` 设为 `none` 时也照常保留；缓冲区仅存在于内存中，服务重启后清空，不能替代上述持久化方案。
