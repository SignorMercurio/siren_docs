...
air:
  ...
  # providers.<name> 用于 WebUI AIR、Manual 裸终端和服务端 REPL air 命令
  providers:
    claude:
...
| `air.provider` | string | `claude` | WebUI AIR 与 Agent 终端默认 Provider，可选 `claude` 或 `codex`；页面中也可在每次启动会话时选择。服务端 REPL 的 `air` 命令固定使用 Claude Provider 配置 |
...
<Callout title="AIR 配置边界" type="info">
  AIR 现在只保留 Provider、prompt、CLI 路径和额外参数。模型、权限、沙箱、普通 Shell 审批等行为由 Claude Code / Codex 自身配置决定；Codex Provider 会把 SIREN 注入的 MCP 工具设置为默认允许。旧版 AIR 细分字段和独立的 `agentTerminal.*` 配置块已移除，残留时不会生效。
</Callout>
