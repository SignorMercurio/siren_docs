...
| `recon.profiles.<name>` | list | 见示例 | 命名的模块集合；只选择采集模块，不覆盖规则、超时或检测逻辑。`default` 为完整采集，`quick` 只移除 `file` |
...
| `rules` | map | - | 白名单等环境基线规则，值为正则表达式 |

<Callout title="关于 rules" type="info">
  Recon 只使用相关白名单减少已知正常项带来的噪音，不把 `rules` 当作黑名单或签名检测引擎。矿工、Webshell、Rootkit symbol 等判断由 AIR 结合上下文完成。
</Callout>
...
没有 `recon.profiles` 的旧配置会把已启用的 `recon.<module>` 与旧式 recon 插件合成为唯一的 `default`，并记录迁移提示。新旧结构同时存在时，以 Profile 为准，旧键被忽略。无效的远程 `reload` 会保留上一个有效 Profile 快照。
