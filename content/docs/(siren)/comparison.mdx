...
这页用于说明 SIREN 和常见主机排查、DFIR 工具的定位差异。对比重点不是功能数量，而是使用场景：一次性排查、远程应急、持续终端取证、还是可编程系统情报查询。
...
[GScan](https://github.com/grayddq/GScan) 是面向 Linux 主机排查的开源 checklist 工具，目标是发现入侵痕迹并辅助梳理攻击路径。SIREN 的早期设计受到了 GScan 启发，但后续重点放在跨平台客户端、低依赖部署、远程模式、Web 控制台和 AI 辅助分析上。
...
[osquery](https://osquery.io) 把操作系统状态抽象成 SQL 表，适合用统一查询语言读取进程、网络连接、内核模块、文件哈希等信息。它适合有现成查询库、Fleet 或其他管理平台的团队，也适合把主机状态查询纳入检测和监控体系。
