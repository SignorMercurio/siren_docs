...
| `MsgRecon` | v2 请求包含 `format=snapshot-jsonl-v1`、客户端采集预算，并支持按 request ID 取消；成功结果包含 Profile、format、`complete|partial` 和 JSONL 文件名，随后紧跟一个 `MsgFile`。v1 继续传 Markdown，旧协议仍使用空请求和文件名响应 |
