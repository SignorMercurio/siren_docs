...
主通道建立后，客户端必须先发送 `MsgClientHello`。服务端通过该帧完成客户端注册、恢复备注与项目归属，并缓存当前插件列表及 Recon Profile 能力。后续插件发生安装、卸载、更新或启停变化时，客户端通过 `MsgPluginRegister` 刷新完整插件列表；结构化 `reload` 后通过 `MsgClientCapabilities` 刷新 Recon 能力。
...
| `MsgRecon` | 新协议使用带 `version`、`requestId`、`profile` 的 JSON 请求/结果；成功结果后紧跟 `MsgFile`。旧协议仍使用空请求和文件名响应 |
...
| `MsgClientCapabilities` | Client -> Server | reload 后刷新 Recon 协议版本、默认 Profile 和名称列表 |
...
sequenceDiagram
    ...
    participant C as Client
    participant S as Server
    S->>C: MsgRecon {requestId, profile} or MsgUpload
    C-->>C: resolve Profile and collect, or read requested file
    alt structured Recon success
