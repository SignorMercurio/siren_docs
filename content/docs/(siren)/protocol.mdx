...
sequenceDiagram
    ...
    C->>S: TLS connect
    C->>S: MsgClientHello {agentId, os, plugins, recon capabilities, host}
...
sequenceDiagram
    ...
    participant C as Client
    participant S as Server
    S->>C: MsgShell {token, cols, rows}
    C->>S: TLS on main port, ALPN siren-shell/1
    C->>S: single-use token
    C-->>C: start interactive shell
    C->>S: startup status
    loop shell IO
        S->>C: input or resize frame
