...
- **Dossier 编辑**：`.md` / `.markdown` 报告可直接在 Dossier 中打开，适合编辑应急报告正文、截图和结论
...
- 从左侧导航点击 **Dossier**，直接打开编辑器
- 在 Artifacts 中选中 `.md` / `.markdown` 报告后，点击 Dossier 按钮编辑该报告

Dossier 未部署或路径配置错误时，WebUI 会显示错误提示，但不影响其他功能。

<Callout title="常见问题" type="warn">
  - 点击 Dossier 后提示不可用：通常是 Dossier 编辑器文件未部署，或 `webui.dossierDir` 指向了错误目录。
  - Artifacts 中没有 Dossier 按钮：只有 `.md` / `.markdown` 报告支持在 Dossier 中编辑。
...
默认配置会从 `/opt/siren/dossier` 读取 Dossier 编辑器文件。可以在控制主机上构建并复制：
...
如果修改了 `webui.dossierDir`，重启 `siren_server` 后再访问 WebUI。反向代理部署时，建议把同一域名整体转发到 WebUI 端口；如果只代理了部分路径，可能出现 Dossier 页面能打开但报告无法保存的问题。不要把报告文件放进 `webui.dossierDir`，报告仍应保留在 SIREN 服务端工作目录下，并通过 Artifacts 查看。
