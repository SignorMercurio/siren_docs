...
每个客户端可开启多个终端标签。底层通过 WebSocket 桥接到服务端，服务端再以 TLS 转发到受害主机 PTY。切换到其他页面（Clients/Agentic IR/Artifacts）时 Shell 会话保持活动，回到 Shells 页可继续输入。

在 Project 视图下，Shell 只能连接当前 Project 的客户端。切换 Project 或切回 Global 时，WebUI 会关闭不属于新视图的活动 Shell，避免继续操作错误项目中的主机。

### Agentic IR

在浏览器内嵌终端里运行 Claude Code / Codex 完成 [Agentic 应急响应](/air)。支持选择 Claude 或 Codex Provider，新建会话，或恢复当前视图工作目录对应的历史会话。

新建会话可选以下入口模式：

- **Manual**：不带 SIREN 上下文的裸 CLI 终端，用于临时的通用操作
- **Custom**：选择客户端并自由输入应急响应指令
...
会话托管在服务端并持续运行，刷新或切换页面后可重新接入并回放此前输出，进程不中断。除 Manual 外，SIREN 会为会话注入项目作用域的 SIREN MCP 服务，请确认 [MCP 服务](/mcp#配置)已启用；模型、权限、沙箱等沿用大模型 CLI 自身配置。Project 视图下的工作目录、历史恢复和 MCP scope 规则见 [Agentic 应急响应](/air#webui-air)。WebUI 浏览器终端不依赖 tmux，服务端 REPL 的 `air` 命令仍保持原有交互式体验。
