...
服务端顶层 `listen` / `port` 决定 Web 控制台、MCP 和远程客户端共用的 TLS 监听地址。`webui.publicURL` 只用于 WebUI AIR 注入 MCP 地址；它必须是证书覆盖的 HTTPS Origin，服务端本机宜将该域名解析到 loopback 或内网地址。完整配置项见 [配置文件](/config#服务端配置)。
