...
GUI 以时间线交错展示 Agent 回复原文与聚合折叠的操作记录，计划、审批和提问出现在对应位置；输入框下方的状态条显示模型、上下文占用和 Provider 额度。Terminal 保留原有 PTY 行为。两种会话都托管在服务端，刷新浏览器或切换页面后可以重新接入并回放，进程不会因为页面离开而中断。除 Manual 外，会话会使用 SIREN MCP，因此需要启用 [MCP 服务](/mcp#启用与访问)。Project 视图下的工作目录、历史恢复和 MCP 作用域规则见 [Agentic 应急响应](/air#webui-air)。
