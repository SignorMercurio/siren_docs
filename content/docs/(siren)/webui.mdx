...
<Callout title="恢复出站规则" type="info">
  ...
  恢复可能切断客户端回连，需要清理客户端时先执行 Clean，再手动恢复。Clean 回报文件已清理或已安排删除后，页面会为匹配的恢复记录提供快捷入口，不自动执行。**Verify status** 用于核对执行是否结束及当前放行状态；SOAR 执行尚未结束或提交结果不明时禁止重复修改。恢复依据不完整时保留资源并提示人工检查。
</Callout>
