...
<Cards>
  ...
  <Card title="Agentic IR" icon={<Brain />}>启动 Claude / Codex / Qoder 辅助排查会话</Card>
...
<Callout title="部署归属" type="info">
  在 Project 视图下通过 Web 控制台单机或批量部署 SIREN 客户端时，唯一匹配到本次 Allow IP 的新客户端会自动归属到当前 Project。无论从 Web 控制台、服务端 REPL 还是 MCP 发起部署，自动关联成功且尚无备注的客户端都会使用 ECS Instance ID 作为备注；批量部署显式填写 Note 时会优先使用该内容。已有备注始终保留，只有显式 Note 与已有备注冲突时才会显示 warning。服务端 REPL 部署不带 Project 上下文，仍保持全局行为。
</Callout>
...
Agentic IR 页面通过 Claude Code、Codex 或 Qoder 完成 [Agentic 应急响应](/air)。所有新会话默认使用结构化 GUI 时间线，也可以在启动前选择 Terminal，使用 Provider 的真实 TUI、斜杠命令和自定义 statusline。

新建会话可选以下入口模式：

- **Manual**，不带客户端、SIREN MCP 或种子 prompt 的空白会话，适合临时通用操作。
...
GUI 以时间线交错展示 Agent 回复原文与聚合折叠的操作记录，计划、审批和提问出现在对应位置；输入框下方的状态条显示模型、上下文占用和 Provider 额度。Agent 运行时仍可发送补充要求调整后续方向，也可以单独停止当前工作；调整方向不会撤销已经完成的命令、文件修改或远程操作。Terminal 保留原有 PTY 行为。两种会话都托管在服务端，刷新浏览器或切换页面后可以重新接入并回放，进程不会因为页面离开而中断。除 Manual 外，会话会使用 SIREN MCP，因此需要启用 [MCP 服务](/mcp#启用与访问)。Project 视图下的工作目录、历史恢复和 MCP 作用域规则见 [Agentic 应急响应](/air#webui-air)。
...
MCP `run` 会写入共享 `audit_id` 的 `mcp.run.requested` 和 `mcp.run.finished` 记录。选择 `mcp` 组件后，可以直接搜索完整命令、返回给 Agent 的有界结果、客户端 ID、outcome 或 `audit_id`；策略拦截、目标拒绝、发送失败、取消、超时和远端执行失败也会留下完成记录。
...
- **Preferences**：偏好只保存在当前浏览器，不跟随账号，也不会同步到其他设备。
  ...
  - Agentic IR 默认 Provider：沿用上次使用的 Provider，或固定 Codex / Claude / Qoder。固定期间仍可为单次会话临时切换；所选 Provider 不可用时会自动使用可用的 Provider。
