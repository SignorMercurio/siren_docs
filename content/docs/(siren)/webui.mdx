...
Web 控制台是 SIREN Server 的浏览器入口。它覆盖日常远程操作：Project 工作区、客户端列表、浏览器 Shell、Agentic IR、调查任务、文件管理、Artifacts、服务端日志、Dossier 和全局设置。需要在控制主机终端里直接操作时，可以改用 [服务端 REPL](/remote)。

桌面侧栏默认显示图标，悬停或键盘聚焦时可查看名称。使用展开／折叠按钮切换导航宽度，选择会保存在当前浏览器；移动端继续使用完整抽屉导航。
...
左侧导航按用途分为四组：**Respond**（Clients、Agentic IR）、**Tools**（Shells、Files、Investigations）、**Evidence**（Artifacts、Reports）和 **System**（Logs）。每个页面都有自己的 URL，例如 `#/investigations/recon` 或 `#/artifacts/<路径>?project=<id>`；刷新、浏览器前进后退和分享链接都会回到同一页面、同一工具标签、同一 AIR 会话或同一选中文件，浏览器标签页标题也随页面变化。
...
<Cards>
  ...
  <Card title="Agentic IR" icon={<Brain />}>启动 Codex / Claude / Pi / Qoder 辅助排查会话</Card>
  ...
  <Card title="Reports" icon={<BookOpenText />}>列出当前范围内的 Dossier 应急报告，并在 Dossier 中打开编辑</Card>
...
<Callout title="恢复出站规则" type="info">
  ...
  恢复可能切断客户端回连，需要清理客户端时先执行 Clean，再手动恢复。Clean 回报文件已清理或已安排删除后，页面会为匹配的恢复记录提供快捷入口，不自动执行。仅当记录包含尚未确认结果的 SOAR 执行 ID 时，显示 **Recheck execution**，用于重新查询当前记录的执行结果，不检查网络连通性或重新提交修改。缺少执行 ID 时，页面会显示具体步骤并提示人工核对 SOAR 执行历史；明确被限流拒绝的提交会提示通过 Restore 处理此前已完成的变更。恢复前仍会核对同账号的未完成操作，遇到阻塞时显示对应实例和操作。恢复依据不完整时保留资源并提示人工检查。
</Callout>

<Callout title="Provisioning 输入与结果" type="info">
...
Agentic IR 页面通过 Codex、Claude Code、Pi 或 Qoder 完成 [Agentic 应急响应](/air)。所有新会话默认使用结构化 GUI 时间线，也可以在启动前选择 Terminal，使用 Provider 的真实 TUI、斜杠命令和自定义 statusline。

新建会话可选以下入口模式：

- **Manual**，不带客户端或种子 prompt 的空白会话，适合临时通用操作；GUI 和 Terminal 均会自动注入当前范围的 SIREN MCP，无需在 Provider 的静态配置中重复添加同名服务。
...
GUI 以时间线交错展示 Agent 回复原文与聚合折叠的操作记录，计划、审批和提问出现在对应位置；输入框下方的状态条显示模型、上下文占用和 Provider 额度。支持模型切换的 Provider 可在空闲时点击状态条中的模型名称选择模型；Codex 还可调整当前模型支持的推理强度。Pi 列出服务器已配置的模型，新会话沿用操作者的思考默认设置，恢复会话保留之前的模型和思考等级。时间线支持 Cmd/Ctrl+F 检索与过滤，并可从状态条把当前会话导出为 Markdown。Agent 运行时仍可发送补充要求调整后续方向，也可以单独停止当前工作；调整方向不会撤销已经完成的命令、文件修改或远程操作。GUI 与 Terminal 都支持直接粘贴 PNG、JPEG、WebP 和 GIF 图片，单张最多 8 MiB、一次最多 4 张；SIREN 负责把图片转交给 Provider，具体模型能否识别由 Provider 和模型决定。Terminal 保留原有 PTY 行为。两种会话都托管在服务端，刷新浏览器或切换页面后可以重新接入并回放，进程不会因为页面离开而中断。所有入口都会使用 SIREN MCP，因此需要启用 [MCP 服务](/mcp#启用与访问)。Project 视图使用项目级 MCP，Global 视图使用全局 MCP；工作目录、历史恢复和 MCP 作用域规则见 [Agentic 应急响应](/air#webui-air)。

GUI 回复中的编号脚注可点击打开来源卡片，查看证据摘要、数据来源、采集时间和原文入口；每条回复下方的 **Sources** 可展开全部来源。文件链接始终打开该会话所属 Project 的 Artifacts。Agent 会优先引用已有 Recon 报告、JSONL 等材料，并在现有权限允许时将重要的临时命令或查询原文保存到工作区的 `evidence/`。材料未取得、无法定位或保存失败时，来源说明会标明缺口。来源由 Agent 提供，卡片不自动认证证据真实性。

Markdown 报告中的证据链接按报告所在目录解析，限定在同一工作区内。会话导出保留标准 Markdown 脚注，并区分各条消息的编号；历史消息不会自动补齐来源。
...
Reports 页面列出当前范围内符合 Dossier 规范的 Markdown 应急报告。Global 视图汇总服务端工作目录和所有 Project 目录，并显示每份报告所属的 Project；Project 视图只列出该 Project 目录内的报告。

- 只有 frontmatter 同时包含 `date`、`sir-seq`、`version` 和 `client` 的 `.md` 文件会被列出，Recon 派生报告和普通笔记不会混入。表格显示客户名称、文件路径、服务编号、版本和修改时间，按报告日期倒序排列。
- 扫描只在首次进入某个范围时执行一次，之后不会自动刷新；新建或修改报告后点击 **Rescan** 重新扫描。扫描跳过隐藏目录、`node_modules`、`.assets` 资源目录和符号链接，并限制深度与条目数，超出时显示 **partial scan**。
- 点击行尾 **Open in Dossier** 在新标签页中打开该报告编辑；Project 报告会保留在其 Project 目录内。
- 点击 **Blank template** 以只读方式打开默认模板 `webui.dossierDir/report.md`。可以查看和试改，但不能保存回该模板；实际报告需在 Artifacts 中创建 `.md` 文件后进入 Dossier 保存。

Dossier 是独立的 Markdown 报告编辑器，也可以在 Artifacts 中选中 `.md` 报告后点击 Dossier 按钮打开。Dossier 未部署、路径配置错误或默认 `report.md` 缺失时，Reports 页面仍会列出报告，但会显示错误提示并禁用打开按钮，不影响其他功能。

详细部署、配置、语法和编辑流程见 [Dossier 文档](/dossier)。

<Callout title="常见问题" type="warn">
  - Reports 页面提示 Dossier 不可用：通常是 Dossier 编辑器文件未部署，或 `webui.dossierDir` 指向了错误目录。
  - **Blank template** 不可用：确认 `webui.dossierDir/report.md` 存在。`webui.dossierDir` 可以使用绝对路径，也可以放在 `siren_server` 工作目录外。
  - 报告没有出现在 Reports 中：确认文件以 `.md` 结尾、frontmatter 位于文件开头且四个字段齐全，然后点击 **Rescan**。
  - 需要保存报告：先在 Artifacts 中创建或选择实际的 `.md` 报告，再从该文件或 Reports 页面进入 Dossier；默认模板不能被覆盖。
...
- **Preferences**：偏好只保存在当前浏览器，不跟随账号，也不会同步到其他设备。
  ...
  - Agentic IR 默认 Provider：沿用上次使用的 Provider，或固定 Codex / Claude / Pi / Qoder。固定期间仍可为单次会话临时切换；所选 Provider 不可用时会自动使用可用的 Provider。
...
<Callout title="公网暴露场景" type="warn">
  ...
  - 使用浏览器信任、且覆盖访问域名的证书
  - 通过防火墙、安全组、VPN 或 ACL 限制主 TLS 端口的访问来源；`/mcp` 使用独立 Bearer 凭证，不使用 WebUI Session 鉴权
