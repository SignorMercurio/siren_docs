...
- **Recon**，选择在线客户端及其实际公布的 Profile 后下发信息收集命令；旧客户端只提供 **Legacy default**。页面显示实际 Profile 和本次 operation 的阶段，例如 `Recon.Basic`、`Recon.User`、`Recon.Process` 和 `Save report to Artifacts`。同一客户端已有 Recon 运行时会直接提示冲突，客户端错误、断连或超时也会结束当前状态，不会一直等待。
